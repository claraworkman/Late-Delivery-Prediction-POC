...
┌─────────────────┐
│ Fabric Semantic │
│     Model       │ ◄─── DLV Aging Columns & Measures
│  (Aging Table)  │
└────────┬────────┘
         │
...
┌─────────────────┐
│  Fabric Notebook│
│  Data Prep      │ ◄─── Closed deliveries (GI Date populated)
│  Exploration    │      Target: AGE_REQ_DATE
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
│                 │      - Classification (late/on-time)
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  MLflow Model   │
│   Registry      │ ◄─── POC-LateDelivery-Regression-AutoML
│                 │      POC-LateDelivery-Classification-AutoML
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Batch Scoring   │
│   Pipeline      │ ◄─── Open deliveries (GI Date blank)
│                 │      + Risk scores + Buckets + Priority flags
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Fabric Lakehouse│
│ (Delta Table)   │ ◄─── late_delivery_predictions
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│   Power BI      │
│    Report       │ ◄─── Direct Lake mode + DAX measures
└─────────────────┘
```
